...
def get_cached_similarity_tables(species, type, restric_to_reliable_genes):
    ...
    if (species, type, restric_to_reliable_genes) in cached_similarity_tables:
        ...
        similarity_file = open(r'gossto/gene_similarities/%s_%s_ism_%s.txt' % ('GOOD' if restric_to_reliable_genes else 'FULL', get_organism(species), type.upper()), 'r')
...
if __name__ == '__main__':
    print preprocess('/Users/fatemeh/Documents/comparison_1/RESULTS/ecoli_core/poolman/final_modules_0.5.txt','ecoli_core','go_distance_bp_F',False)
    #species_filt = None
